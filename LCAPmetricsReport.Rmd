...
<summary>[**Click here to display table**]{style="color:blue;"}</summary>

```{r reds_pivot, echo=FALSE, include=TRUE, results = 'asis'}
...
lcap.reds.pivot <- lcap.reds.pivot %>%
    filter(districtname == dist.name,
           
           ) %>% 
     select(-cds, -districtname) %>%
    arrange(studentgroup.long)


gt::gt(lcap.reds.pivot) |>
        tab_header(title = "Red Student Groups and Indicators Clustered") |>
      tab_source_note(source_note = "Source: https://www.caschooldashboard.org/" ) |>
  cols_label(
    studentgroup.long = html("Student Group"),
 #   indicator = html("Indicator"),
