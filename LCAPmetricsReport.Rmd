...
---
title: `r metrics_dist[4,2]` 2024-25 LCAP Metrics Report
---
...
Please note that the most recently available data is used, with a preference for the California Dashboard.  Additional data represented comes from DataQuest.  Most data reflect the 2022-23 school year, but in some case may represent the current school year or prior years. MCOE and CDE also encourage you to use current local data from your School Information System or other sources.  
...
### Reds on the Dashboard 

LEAs are required to include actions for every Red indicator on their Dashboard for student groups, schools, or stuent groups at schools.  
...
lcap.reds <- lcap.reds %>%
    filter(districtname == dist.name,
     ...
     select(indicator, studentgroup.long, schoolname) %>%
...
Districts that are eligible Differentiate Assistance need to provide a summary of the work underway as part of receiving technical assistance. 
...
da.need <- if ( length( da.dist$studentgroup.long) > 0 ) {
  ...
  cols_label(
    ...
    } else "The District is not eligible for DA"
...
if ( length( da.dist$studentgroup.long) > 0 ) {
    da.need
    } else print("The District is not eligible for DA")
...
For CSI identified schools, it is required to address the CSI prompt in the LCAP template. 
...
req.goals.student2 <- req.goals.student %>%
    filter(CDS == dist) %>%
    select(groups = `Targeted Student Group(s)`)

req.goals <- if ( length( req.goals.student2$groups) > 0 ) req.goals.student2$groups else
                       "No student groups"
...
`r req.goals` are required to have their own goal on your LCAP.  

In addition, if you have an equity multiplier schools be sure to: xxx. 

Please see https://www.cde.ca.gov/fg/aa/lc/equitymultiplier.asp for additional details. 
...
el.words2 <- if ( el.lang$el.perc >= .15 | el.lang$el.count >=50 ) " Your district is required to have a DELAC." else " Your district is NOT required to have a DELAC."
...
1. Reflect on each of the indicators.  It is highly recommended to use the self-reflection tools provided by the CDE at https://www.cde.ca.gov/ta/ac/cm/localindicators.asp
